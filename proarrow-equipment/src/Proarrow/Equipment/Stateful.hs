...
import Proarrow.Promonad.Writer (Writer, pattern Writer)
...
data StT a b where
  StT :: (Strong Tensor p, Strong Tensor q) => p :~> q -> StT (ST p :: STT' k i j) (ST q)
instance (SymMonoidal k) => Profunctor (StT :: CAT (STT' k i j)) where
  dimap = dimapDefault
  r \\ StT p = r \\ p
instance (SymMonoidal k) => Promonad (StT :: CAT (STT' k i j)) where
  id = StT id
  StT f . StT g = StT (f . g)
instance (SymMonoidal k) => CategoryOf (STT' k i j) where
  type (~>) = StT
  type Ob (a :: STT' k i j) = (Is ST a, Strong Tensor (UN ST a))

instance (SymMonoidal k) => Bicategory (STT' k) where
  type I = ST Id
...
instance
  (SymMonoidal k, Adjunction (PK l) (PK r), Ob l, Ob r, Strong Tensor r, Strong Tensor l)
...
instance (SymMonoidal k) => IsWriter (Id :: k +-> k) where type WithReader Id = Id
instance (SymMonoidal k, Ob (a :: k)) => IsWriter (Writer a :: k +-> k) where
...
instance (SymMonoidal k) => IsReader (Id :: k +-> k) where type WithWriter Id = Id
instance (SymMonoidal k, Ob (a :: k)) => IsReader (Reader (OP a) :: k +-> k) where
...
instance (SymMonoidal k) => WithObO2 Tight (STT' k) where withObO2 r = r
instance (SymMonoidal k) => WithObO2 Cotight (STT' k) where withObO2 r = r

-- | Stateful transformers.
-- https://arxiv.org/pdf/2305.16899 definition 6
-- Generalized to any symmetric monoidal action.
instance (SymMonoidal k) => Equipment (STT' k) where
...
crossing
  :: forall {k} (p :: k +-> k) (a :: k)
   . (Strong Tensor p, Ob a, SymMonoidal k) => STSq p p a a
crossing = StT \(Writer g :.: p) -> lmap g (act @Tensor @p @a p) :.: Writer (obj @a ** tgt p) \\ p

pi0
  :: forall {k} (p :: STT k) (q :: STT k). (Ob p, Ob q, SymMonoidal k) => ST (UN ST p :*: UN ST q) ~> p
pi0 = StT \(p :*: _) -> p

pi1
  :: forall {k} (p :: STT k) (q :: STT k). (Ob p, Ob q, SymMonoidal k) => ST (UN ST p :*: UN ST q) ~> q
pi1 = StT \(_ :*: q) -> q

mult
  :: forall {k} (p :: STT k) q r (a :: k) b
   . (Ob p, Ob q, Ob r, SymMonoidal k, Ob a, Ob b)
  => p ~> q -> p ~> r -> p ~> ST (UN ST q :*: UN ST r)
StT n `mult` StT m = StT \p -> n p :*: m p

ip0
  :: forall {k} (p :: STT k) q. (Ob p, Ob q, SymMonoidal k) => p ~> ST (UN ST p :+: UN ST q)
ip0 = StT \p -> InjL p

ip1
  :: forall {k} (p :: STT k) q. (Ob p, Ob q, SymMonoidal k) => q ~> ST (UN ST p :+: UN ST q)
ip1 = StT \p -> InjR p

-- plus
--   :: forall {k} (p :: k +-> k) q r (a :: k) b
--    . (Strong Tensor p, Strong Tensor q, Strong Tensor r, M.SymMonoidal m, Ob a, Ob b)
--   => STSq p r a b -> STSq q r a b -> STSq (p :+: q) r a b
-- STSq n `plus` STSq m = STSq (coproduct n m)

-- sigma0
--   :: forall {k} (a :: k) b
--    . (Ob a, Ob b, M.SymMonoidal m, SymMonoidal k, HasCoproducts m) => STSq (Id :: k +-> k) Id a (a || b)
-- sigma0 = withObCoprod @m @a @b $ STSq \(Id f) -> Id (act (lft @m @a @b) f)

-- sigma1
--   :: forall {k} (a :: k) b
--    . (Ob a, Ob b, M.SymMonoidal m, SymMonoidal k, HasCoproducts m) => STSq (Id :: k +-> k) Id b (a || b)
-- sigma1 = withObCoprod @m @a @b $ STSq \(Id f) -> Id (act (rgt @m @a @b) f)

-- copair
--   :: forall {k} (p :: k +-> k) q (a :: k) b c
--    . (Strong Tensor p, Strong Tensor q, M.SymMonoidal k, Distributive k, SymMonoidal k, MonoidalProfunctor (Coprod q), Ob a, Ob b, Ob c)

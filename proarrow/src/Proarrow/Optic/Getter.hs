...
-- pair ('ExOptic' 'GetterRes', via 'withLegs') and read the get leg off with 'getP'.
view
  :: forall {j} {k} c (s :: k) (t :: j) a b
   . (CategoryOf j, CategoryOf k, (Ob a, Ob b) => c (ExOptic GetterRes a b))
  => Optic c s t a b -> s ~> a
view o = withLegs @GetterRes o \ @p @q p _ -> getP @p @q p

infixl 8 ^.

-- | View the focus of a concrete, @Type@-level optic.
(^.) :: (c (ExOptic GetterRes a b)) => s -> Optic c (s :: Type) (t :: Type) a b -> a
s ^. l = view l s

to :: forall {k} {j} (s :: k) (t :: j) a b. (CategoryOf k, CategoryOf j, Ob b, Ob t) => (s ~> a) -> Getter s t a b
to sa = legs2prof @GetterRes (Id sa) TerminalProfunctor \\ sa
...
review
  :: forall {j} {k} c (s :: k) (t :: j) a b
   . (CategoryOf j, CategoryOf k, (Ob a, Ob b) => c (ExOptic (Flip GetterRes) a b))
  => Optic c s t a b -> b ~> t
review o = withLegs @(Flip GetterRes) o \ @p @q _ q -> getP @q @p q

infixr 8 #

-- | Review through a concrete, @Type@-level optic.
(#) :: (c (ExOptic (Flip GetterRes) a b)) => Optic c (s :: Type) (t :: Type) a b -> b -> t
(#) = review

unto :: forall {k} {j} (s :: k) (t :: j) a b. (CategoryOf k, CategoryOf j, Ob s, Ob a) => (b ~> t) -> Review s t a b
unto bt = legs2prof @(Flip GetterRes) TerminalProfunctor (Id bt) \\ bt

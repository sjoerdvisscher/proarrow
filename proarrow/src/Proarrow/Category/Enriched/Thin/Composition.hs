...
-- | As much of a quantale as the closure needs, and a totally ordered, integral one at that: a
-- 'Semicartesian' monoidal category, so that the unit is the top element, with binary joins and a
-- bottom, in which the join of two objects is one of them ('minIs'). Totality is what makes a single
-- best walk exist ('shortest'); in a quantale of, say, sets of paths, a join is attained by no one
-- summand. Distributivity of the tensor over joins, the remaining quantale axiom, is not needed by
-- the closure, and infinite joins are not needed since there are finitely many objects.
class (Semicartesian v, HasBinaryCoproducts v, HasInitialObject v) => Quantale v where
...
splitUnit f =
  ( rightUnitor @v @x . (obj @x ** terminate @v @y) . f
  , leftUnitor @v @y . (terminate @v @x ** obj @y) . f
  )
...
-- | Distances are compared with 'cmpNat', whose evidence makes the 'Min' in the join reduce.
...
-- zero-cost pieces to a zero distance, and the actions of the base are the triangle inequality. The
-- inequality holds for shortest paths but not by construction, so, as elsewhere in
-- "Proarrow.Category.Instance.Cost", it is checked at runtime ('triangle').
instance
  ...
  rmap @a @b @c =
    withProObj @COST @(Hom k) @b @c
      ( withProObj @COST @(Walk n p) @a @b
          (withProObj @COST @(Walk n p) @a @c (triangleAt @(HomObj COST b c) @(Walks COST n p a b) @(Walks COST n p a c)))
      )
  lmap @a @b @c =
    withProObj @COST @(Hom k) @c @a
      ( withProObj @COST @(Walk n p) @a @b
          (withProObj @COST @(Walk n p) @c @b (triangleAt @(HomObj COST c a) @(Walks COST n p a b) @(Walks COST n p c b)))
      )

-- | An arrow of 'COST' between distances known only by their singletons. For shortest paths it is
-- the triangle inequality, which holds but not by construction, so it is checked at runtime.
triangle :: forall (x :: COST) y. (IsCost x, IsCost y) => x ~> y
triangle = case (sing @x, sing @y) of
  (SINF, _) -> Inf
  (SC @m, SC @m') -> case cmpNat (Proxy @m') (Proxy @m) of
    LTI -> GTE
    EQI -> GTE
    GTI -> error "Walk: triangle inequality violated"
  (SC, SINF) -> error "Walk: triangle inequality violated"

-- | 'triangle' out of a tensor of two distances.
triangleAt :: forall (x :: COST) y z. (IsCost x, IsCost y, IsCost z) => x ** y ~> z
triangleAt = withOb2 @COST @x @y (triangle @(x ** y) @z)

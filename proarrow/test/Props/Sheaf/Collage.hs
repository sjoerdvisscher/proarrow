...
-- | __A site that is neither a poset nor overlap-free.__ Every other coverage in @Props.Sheaf@ is
-- one or the other: @Overlapping@ has legs that meet, on a poset;
-- @Examples.Graph@'s @ByEnds@ has parallel legs, with nothing in the overlap. Matching is what
-- needs both -- a family has to agree on the overlaps, and which arrow it agrees along only
-- matters when there is more than one.
--
-- The category is the collage of 'Pair': the diamond of opens on the left, one extra object on the
-- right, and the elements of 'Pair' as the arrows between the layers. That makes the cross-arrows
-- parallel (two of them out of each open) while the overlap comes from the diamond, which is the
-- only way round -- a collage's cross-arrows all go one way, so they cannot form an overlap by
-- themselves.
...
-- Restriction to the intersection keeps the label, so the two @X@s agree there and the two @Y@s
-- do -- that agreement is the overlap the site is built for. Nothing over the top, which is what
-- keeps the top out of the coverage.
...
-- | Comparable and generable at its objects, which is what the generic 'Collage' instances in
-- "Proarrow.Testing" ask of the profunctor a collage is built from.
...
-- is covered by every arrow into it from the diamond. Six legs -- two out of each half and two
-- out of the intersection -- and the last two factor through the first four, so the sieve is the
-- one four generating legs would give.
--
...
-- sheaf is determined by. Carries its objects, as 'Sieve' and 'Plus' do, so that @'Ob' a@ is had
-- rather than recovered from @'Ob' ('L' a)@ -- which 'IsLR' takes as a premise and does not give
-- back.
...
-- | Compared and shown by its index. Within one hom-set the constructor is determined by the
-- objects -- @'L' a '~>' 'L' b@ and @'L' a '~>' 'R' b@ differ in their target -- so the index is
-- faithful, as it is for 'Proarrow.Category.Enriched.Finitary.Topos.Tabulated'.
--
-- Pinned to this collage rather than written for any: 'TestableProfunctor'\'s quantified
-- superclass needs the instance at /abstract/ objects, so it cannot be one instance per @L@\/@R@
-- shape, and a shape-agnostic one has to ask for its components through nested quantified
-- constraints, which GHC will not discharge through the product kind's object decomposition.
...
-- the object itself. It is the witness that matching has content here -- see the test below.
...
test =
  testGroup
    ...
    , -- The point of the site. Two sections of 'AtApex' at each of the six legs is 2^6 families;
        ...
        ( \ @tab _ _ ->
            testGroup
              ...
              , -- The half of "a sheaf is its left layer" that the sheaf condition does not
                -- already give. That a sheaf's value at the extra object is determined by its
                -- left part is what isSheaf checks; that restriction to the left layer loses no
                -- maps either -- every presheaf map extends, and only one way -- is this, and
                -- nothing else here tests it. Together they say the sheaves on a ByElements site
                -- are the presheaves on its left layer, the right one carrying no information.
              ...
              , -- Gluing where matching rules families out: the search has four legs to satisfy
                -- and only a quarter of the families to choose from, which is the case

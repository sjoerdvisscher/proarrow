...
instance (Bicategory kk) => Double (QKK kk) (COK kk) where
  ...
  (|||)
    :: forall {h} {j} (ps :: Path (QKK kk) h j) qs rs fs gs hs is
     . Quintet ps qs fs gs -> Quintet qs rs hs is -> Quintet ps rs (fs +++ hs) (gs +++ is)
  Q (Q1 n) ||| Q (Q1 m) =
    appendObj @fs @hs $
      appendObj @gs @is $
  ...
  (===)
    :: forall {h} {i} (ps :: Path (QKK kk) h i) qs rs ss fs gs hs
     . Quintet ps qs fs gs -> Quintet rs ss gs hs -> Quintet (ps +++ rs) (qs +++ ss) fs hs
  Q (Q1 n) === Q (Q1 m) =
    appendObj @ps @rs $
      appendObj @qs @ss $
        let fs = obj @(UN CO (Fold fs))

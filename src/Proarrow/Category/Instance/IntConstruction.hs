  ...
  Int @bp @bm @cp @cm f . Int @ap @am g =
    Int
      ( trace @_ @(bp ** bm) @(ap ** cm) @(am ** cp)
          (swapOuter @bm @cp @bp @am . (f `par` (swap @am @bp . g)) . swapFst @ap @cm @bp @bm)
      )
      \\ obj2 @ap @cm
      \\ obj2 @am @cp
      \\ obj2 @bp @bm
...
instance (TracedMonoidal k, Ob (Unit :: k)) => Monoidal (INT k) where
  ...
  leftUnitor @(I ap am) =
    Int ((leftUnitorInv @k @am `par` obj @ap) . swap @ap @am . (leftUnitor @k @ap `par` obj @am))
      \\ obj2 @Unit @ap
      \\ obj2 @Unit @am
  leftUnitorInv @(I ap am) =
    Int ((obj @am `par` leftUnitorInv @k @ap) . swap @ap @am . (obj @ap `par` leftUnitor @k @am))
      \\ obj2 @Unit @ap
      \\ obj2 @Unit @am
  rightUnitor @(I ap am) =
    Int ((rightUnitorInv @k @am `par` obj @ap) . swap @ap @am . (rightUnitor @k @ap `par` obj @am))
      \\ obj2 @ap @Unit
      \\ obj2 @am @Unit
  rightUnitorInv @(I ap am) =
    Int ((obj @am `par` rightUnitorInv @k @ap) . swap @ap @am . (obj @ap `par` rightUnitor @k @am))
      \\ obj2 @ap @Unit
      \\ obj2 @am @Unit
  associator @(I ap am) @(I bp bm) @(I cp cm) =
    Int (swap @(ap ** (bp ** cp)) @((am ** bm) ** cm) . (associator @k @ap @bp @cp `par` associatorInv @k @am @bm @cm))
      \\ obj2 @(I ap am) @(I bp bm) `par` obj @(I cp cm)
      \\ obj @(I ap am) `par` obj2 @(I bp bm) @(I cp cm)
  associatorInv @(I ap am) @(I bp bm) @(I cp cm) =
    Int (swap @((ap ** bp) ** cp) @(am ** (bm ** cm)) . (associatorInv @k @ap @bp @cp `par` associator @k @am @bm @cm))
      \\ obj2 @(I ap am) @(I bp bm) `par` obj @(I cp cm)
      \\ obj @(I ap am) `par` obj2 @(I bp bm) @(I cp cm)

instance (TracedMonoidal k, Ob (Unit :: k)) => SymMonoidal (INT k) where
  Int @ap @am @bp @bm f `swap'` Int @cp @cm @dp @dm g =
    Int (swapSnd @am @bp @dp @cm . (f `par` (swap @cm @dp . g . swap @dm @cp)) . swapSnd @ap @cp @dm @bm)
      \\ obj2 @(I ap am) @(I cp cm)
...
instance (TracedMonoidal k, Ob (Unit :: k)) => StarAutonomous (INT k) where
  ...
  dual (Int @ap @am @bp @bm f) = Int (swap @am @bp . f . swap @bm @ap)
  dualInv (Int @ap @am @bp @bm f) = Int (swap @am @bp . f . swap @bm @ap)
...
instance (TracedMonoidal k, Ob (Unit :: k)) => CompactClosed (INT k) where
  distribDual @(I ap am) @(I bp bm) = Int (swap @(am ** bm) @(ap ** bp)) \\ obj2 @(I ap am) @(I bp bm)
  dualUnit = id
